...
(function() {
     ...
     * @param {string} 推流ur 可选  
     * @param {object} viewOptions 初始化view对象
     *                   viewOptions = {                            
     *                       videoWidth: ,//int (optional) 视频宽 default 640
     *                       videoHeight: ,//int (optional) 视频高 default 480
     *                       fps: , //number (optional) 帧率 default 15
     *                       bitrate: //number (optional) 码率 default 600
     *
     * @param {object} flashOptions 初始化flash对象
     *                   flashOptions = {
     *                       previewWindowWidth: ,//number (optional) default 862
     *                       preViewWindowHeight: ,//number (optional) default 446
     *                       wmode: ,//string (optional) "window"模式 默认显示方式下flash总是会遮住位置与他重合的所有DHTML层
     ...
     */
    function nePublisher(container) {
        var i = 1;
        var url, viewOptions, flashOptions, initSuccessCallbackFun, initErrorCallbackFun;
        this.initPublish = {
        ...
        if (typeof arguments[i] === 'function') {
            initSuccessCallbackFun = arguments[i];
            i++;
        }
        if (typeof arguments[i] === 'function') {
            initErrorCallbackFun = arguments[i];
        }
        if (!nePublisher.__id) {
        ...
        this.initSuccessCallbackFun = initSuccessCallbackFun; // 准备成功回调
        this.initErrorCallbackFun = initErrorCallbackFun; // 准备错误回调
        this.errorMessageCallbackFun = null; //推流过程中产生错误信息回调
        // this.errorMessageCallbackFunReady = null; //z准备初始化时产生错误信息回调
        // extend(this.initPublish, viewOptions);
        // extend(this.initPublish, flashOptions);
        this.component = {
            cameraList: [],
            microPhoneList: []
        };
    ...
    nePublisher.prototype.start = function() {
        ...
        flashvars.width = this.previewWindowWidth;
        flashvars.height = this.previewWindowHeight;
        ...
        swfobject.embedSWF( //现在有个问题 此路径是相对于html页面的
            "./js/publisher/src/nePublisher.swf?_version=" + ne_get_version_code(), // swf路径改成cdn路径
            this.container,
            this.previewWindowWidth, this.previewWindowHeight,
            "11.1.0", "./js/publisher/src/AdobeFlashPlayerInstall.swf",
    ...
    /**
     * [startPublish publish stream to server.]
     * @param  {UrlString} url  推流地址
     * @param  {Object} videoOptions 推流配置 设置清晰度
     *                                            options = {
     *                                                videoWidth: ,//int (optional) default 640,
     *                                                videoHeight: ,//int (optional) default 480,
     *                                                fps: ,//number (optional) default 15,
     *                                                bitrate: //number (optional) default 600
     *                                            }
     *     
     * @param  {[function]} errorMessageCallbackFun    [推流过程中发生错误进行回调 函数参数为(错误代码，错误信息)]
     */
    nePublisher.prototype.startPublish = function() {
        ...
        this.callbackObj.ref.__publish(this.url, this.previewWindowWidth, this.previewWindowHeight, this.vcodec, this.acodec);
    };
    ...
    /**
     * [getCameraList 获得摄像头列表]
     * @return {Array} 字符串数组列表
     */
    ...
    /**
     * [setCamera 设置摄像头]
     * @param {number} cameraIndex 摄像头列表的索引
     */
    ...
    /**
     * [getMicroPhoneList 获得麦克风列表]
     * @return {Array} 麦克风数组列表
     */
    ...
    /**
     * [setMicroPhone 设置麦克风]
     * @param number} microPhoneIndex 麦克风列表的索引
     */
    ...
    /**
     * [alterDefinedErrors 修改提示错误信息 错误代码编号不能更改]
     * @param  {object} errorObject 错误对象
     * @return {object} 返回被更改的错误信息
     */
    ...
    /**
     * [resetPreviewWindow 重新设置推流器] 函数有问题初始化完推流器后，无法在修改flash参数
     * @param  {object} flashOptions  初始化flash对象
     */
    nePublisher.prototype.resetPreviewWindow = function(flashOptions) {
        this.previewWindowWidth = flashOptions.previewWindowWidth;
        this.previewWindowHeight = flashOptions.previewWindowHeight;
    };
     ...
     * @param  {array} cameraList cameraList a string array contains the names of cameras.
     * @param  {array} microphoneList microphoneList a string array contains the names of microphones.
     * @param  {function} callbackFun    [description] 推流准备就绪回调函数 可以初始化摄像头，麦克风列表
     */
    nePublisher.prototype.on_publisher_ready = function(cameraList, microPhoneList, initSuccessCallbackFun) {
        this.component.cameraList = cameraList;
        this.component.microPhoneList = microPhoneList;
        initSuccessCallbackFun.apply(this);
    };

    /**
     * [on_publisher_error description]
     * @param  {int} code [the error code.]
     * @param  {string} desc [the error desc message.]
     */
    nePublisher.prototype.on_publisher_error = function(code, desc) {
        this.errorMessageCallbackFun(code, desc);
    };
    nePublisher.prototype.on_publishInit_error = function(code, desc, initErrorCallbackFun) {
        initErrorCallbackFun.apply(this, [code, desc]);
    };
...
function __ne_on_publisher_ready(id, cameras, microphones) {
    ...
    publisher.on_publisher_ready(cameras, microphones, publisher.initSuccessCallbackFun);
}
...
function __ne_on_publishInit_error(id, code) {
    var publisher = __ne_find_publisher(id);
    publisher.on_publishInit_error(code, publisher.errors["" + code], publisher.initErrorCallbackFun);
}
...
function extend(obj, copyObject) {
    var attr,
        src,
        copy;
    for (attr in copyObject) {
        src = obj[attr];
        copy = copyObject[attr];
        if (src === copy) {
            continue;
        }
