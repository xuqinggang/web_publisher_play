package
{
    ...
    public class nePublisher extends Sprite
    {
        // user set id.
        ...
        private var js_on_publisher_ready2:String = null;
        private var js_on_publishInit_error:String = null;
        private var js_on_publisher_warn:String = null;
        private var js_on_publisher_getSDKVersion:String = null;
		...
		private const error_url_error:int = 105;
        
        public function nePublisher()
        {
            Security.allowDomain("*");//允许flash跨域
            if (!this.stage) {
        ...
        private function system_on_add_to_stage(evt:Event):void {
            ...
            this.js_on_publisher_ready2 = flashvars.on_publisher_ready2;
            this.js_on_publisher_error = flashvars.on_publisher_error;
           
            this.js_on_publisher_warn = flashvars.on_publisher_warn;
            this.js_on_publisher_getSDKVersion = flashvars.on_publisher_getSDKVersion;
             this.js_on_publishInit_error = flashvars.on_publishInit_error;
            // initialized size.
        ...
        private function system_on_js_ready():void {
            ...
            flash.external.ExternalInterface.addCallback("__preview", this.js_call_preview);
            
            var cameras:Array = Camera.names;
            var microphones:Array = Microphone.names;
            var sdkVersion:String = "V1.0.1";
            trace("retrieve system cameras(" + cameras + ") and microphones(" + microphones + ")");

            if (!cameras.length) {
                system_error_ready(this.error_camera_get, "");
                flag = 0;
                //return ;
            }

            if (!microphones.length) {
                system_error_ready(this.error_microphone_get, "");
                return ;
            }
            if(!flag) {
                return ;
            ...
            flash.external.ExternalInterface.call(this.js_on_publisher_getSDKVersion, this.js_id, sdkVersion);
        }
        
        /**
        * notify the js an error occur.
        */
        private function system_error_ready(code:int, desc:String):void { //准备时错误回调
            trace("system error, code=" + code + ", error=" + desc);
            flash.external.ExternalInterface.call(this.js_on_publishInit_error, this.js_id, code);
        }
        private function system_error(code:int, desc:String):void { //推流时错误回调
            trace("system error, code=" + code + ", error=" + desc);
        ...
        // ne infos
        private var ne_server:String = null;
        private var ne_primary:String = null;
        private var ne_authors:String = null;
        private var ne_id:String = null;
        private var ne_pid:String = null;
        private var ne_server_ip:String = null;
        private function update_context_items():void {
            // for context menu
            var customItems:Array = [new ContextMenuItem("nePlayer")];
            if (ne_server != null) {
                customItems.push(new ContextMenuItem("Server: " + ne_server));
            }
            if (ne_primary != null) {
                customItems.push(new ContextMenuItem("Primary: " + ne_primary));
            }
            if (ne_authors != null) {
                customItems.push(new ContextMenuItem("Authors: " + ne_authors));
            }
            if (ne_server_ip != null) {
                customItems.push(new ContextMenuItem("neIp: " + ne_server_ip));
            }
            if (ne_pid != null) {
                customItems.push(new ContextMenuItem("nePid: " + ne_pid));
            }
            if (ne_id != null) {
                customItems.push(new ContextMenuItem("neId: " + ne_id));
            }
            contextMenu.customItems = customItems;
        }

        private function js_call_preview(camera_ID:String):void {
            // ignore muted, for flash will require user to access it.
            
            // Remark: the name is the index!
            var camera:Camera = Camera.getCamera(camera_ID);
            if(camera == null){
                this.system_error(this.error_camera_get, "");
                return;
            }
            // ignore muted, for flash will require user to access it.
            // but we still warn user.
            if(camera && camera.muted){
                this.system_warn(this.error_camera_muted, "");
            }

            __show_local_camera(camera);
        }
        ...
        private function js_call_publish(url:String, _width:int, _height:int, vcodec:Object, acodec:Object):void {
			...
			if (url.lastIndexOf("live.126.net") == -1) {
				js_call_stop();
				system_error(error_url_error, "the livestreaming url error!");
				return;
			}
            ...
            this.js_call_stop();
            //flash.external.ExternalInterface.call(this.js_on_publisher_ready2, JSON.stringify(vcodec),JSON.stringify(acodec)+"!!");
            ...
            this.media_conn.addEventListener(NetStatusEvent.NET_STATUS, function(evt:NetStatusEvent):void {
                ...
                if (evt.info.hasOwnProperty("data") && evt.info.data) {
                    if (evt.info.data.hasOwnProperty("ne_server")) {
                        ne_server = evt.info.data.ne_server;
                    }
                    if (evt.info.data.hasOwnProperty("ne_primary")) {
                        ne_primary = evt.info.data.ne_primary;
                    }
                    if (evt.info.data.hasOwnProperty("ne_authors")) {
                        ne_authors = evt.info.data.ne_authors;
                    }
                    if (evt.info.data.hasOwnProperty("ne_id")) {
                        ne_id = evt.info.data.ne_id;
                    }
                    if (evt.info.data.hasOwnProperty("ne_pid")) {
                        ne_pid = evt.info.data.ne_pid;
                    }
                    if (evt.info.data.hasOwnProperty("ne_server_ip")) {
                        ne_server_ip = evt.info.data.ne_server_ip;
                    }
                    update_context_items();
                }
                //flash.external.ExternalInterface.call(this.js_on_publisher_ready2, evt.info.code, "code1");
                if (evt.info.code == "NetConnection.Connect.Closed") {
                  //  flash.external.ExternalInterface.call(this.js_on_publisher_ready2, evt.info.code, "code3");
                    js_call_stop();
                ...
                //flash.external.ExternalInterface.call(this.js_on_publisher_ready2, evt.info.code, "code2");
        ...
        /*    if(vcodec.codec == "vp6"){
            ...
            var x264profile:String =H264Profile.MAIN;
            var x264level:String = "4.1";
            var cameraFps:Number = Number(vcodec.fps);
            var x264KeyFrameInterval:int = int(1 * cameraFps);
            ...
            flash.external.ExternalInterface.call(this.js_on_publisher_ready2, cameraWidth, cameraHeight);
            // use default values.
