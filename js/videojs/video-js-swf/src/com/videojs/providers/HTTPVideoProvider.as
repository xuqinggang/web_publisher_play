package com.videojs.providers{
    ...
    public class HTTPVideoProvider extends EventDispatcher implements IProvider{
        ...
        private var _flv_beginning:int;
        private var _seekSecond:int = 0;
        ...
        public function get buffered():Array{
            //duration = 7482;
            //ExternalInterface.call('videojs.Flash.test',   'buffered1', _metadata.duration);
            if(_ns) {
                if (_src.path === null) {
                    // data generation mode
                    //ExternalInterface.call('videojs.Flash.test',   'buffered');
                    if (_isSeeking) {
                        return [];
                    }
                    return [[
                   ...
                   // ExternalInterface.call('videojs.Flash.test',   'buffered2');
                    ...
                    ExternalInterface.call('videojs.Flash.test',   'buffered1', _metadata.duration);
                    return [[0, _seekSecond + (_ns.bytesLoaded / _ns.bytesTotal) * (duration - _seekSecond) ]];
                }
            }
            //ExternalInterface.call('videojs.Flash.test',   'buffered3');
            return [];
        }
        ...
        public function play():void{
            ...
            //ExternalInterface.call('videojs.Flash.test',   'flash play3');
            if(!_loadStarted){
                _metadata = {};
                ...
                load();
                //ExternalInterface.call('videojs.Flash.test',   'flash play4');
            } else {
               // ExternalInterface.call('videojs.Flash.test',   'flash play51');
        ...
        public function resume():void{
            if(_playbackStarted && _isPaused){
                //ExternalInterface.call('videojs.Flash.test',   'resume');
                _ns.resume();
                _model.broadcastEventExternally(ExternalEventName.ON_RESUME);
                if (!_isBuffering) {
                    _model.broadcastEventExternally(ExternalEventName.ON_START);
        ...
        public function seekBySeconds(pTime:Number):void{
            ...
            //ExternalInterface.call('videojs.Flash.test',   'seeking2', pTime);
            //_ns.seek(pTime);
            manuallySeek(pTime);
        }
        public function manuallySeek(second:Number):void {
            ExternalInterface.call('videojs.Flash.test',   'seeking5', _ns.bytesLoaded,  _ns.bytesTotal);
            var cached_seconds = Math.floor((_metadata.duration - _flv_beginning) *(_ns.bytesLoaded / _ns.bytesTotal)) - 1;
            if(second >= _flv_beginning && second < _flv_beginning + cached_seconds)
                {
                  if(Math.abs(_ns.time - second) > 2)
                  {
                    ExternalInterface.call('videojs.Flash.test',   'seeking3', second);
                    _ns.seek(second);
                  }
                }
                else if(_metadata.keyframes)
                {
                  _seekSecond = second;
                  ExternalInterface.call('videojs.Flash.test',   'seeking4', second);
                  var keyframe = get_nearest_keyframe(second, _metadata.keyframes.times);
                  //trace("seek: nearest keyframe=" + keyframe);

                  second = _metadata.keyframes.times[keyframe];
                  if(_flv_beginning != second)
                  {
                    _flv_beginning = second;
                    var url = _src.path +
                              "?start=" + _metadata.keyframes.filepositions[keyframe];
                    //trace("net_stream_.play(" + url + ")");
                    _ns.play(url);
                    //ExternalInterface.call('videojs.Flash.test',   'seeking5155', _metadata.duration, _ns.bytesLoaded,  _ns.bytesTotal);
                  }
                }
                /*
                else
        ...
        public function seekByPercent(pPercent:Number):void{
            if(_playbackStarted && _metadata.duration != undefined){
                ...
                if(pPercent < 0){
                    ...
                    manuallySeek((pPercent / 100) * _metadata.duration);
                }
                else{
                    _throughputTimer.stop();
                    //_ns.seek(pPercent * _metadata.duration);
                    manuallySeek(pPercent * _metadata.duration);
        ...
        private function calculateThroughput():void{
          ...
          if(_ns){
              ...
              if(_ns.bytesLoaded == _ns.bytesTotal){
                  ...
                  //ExternalInterface.call('videojs.Flash.test',   'xqg1');
              }
              ...
              // won't ever cause this logic to run :(
                  ...
                  //ExternalInterface.call('videojs.Flash.test',   _ns.bytesLoaded,_metadata.duration,_ns.bytesTotal,'xqg2211');

                  _currentThroughput = _ns.bytesLoaded / ((getTimer() - _loadStartTimestamp) / 1000);//每秒钟加载数据速率 B/s
                  //var __estimatedTimeToLoad:Number = (_ns.bytesTotal - _ns.bytesLoaded) * _currentThroughput;//预期加载完字节的时间 s
                  var __estimatedTimeToLoad:Number = (_ns.bytesTotal - _ns.bytesLoaded) / _currentThroughput;//xqg调整：改成除（预期加载完字节的时间 s）
                  ExternalInterface.call('videojs.Flash.test',   __estimatedTimeToLoad, _metadata.duration, 'xqg221', _currentThroughput);
                  if(__estimatedTimeToLoad <= _metadata.duration){
                    ExternalInterface.call('videojs.Flash.test',   __estimatedTimeToLoad, _metadata.duration, 'xqg222');
                      _throughputTimer.stop();
                      _throughputTimer.reset();
                      _canPlayThrough = true;
                      _model.broadcastEventExternally(ExternalEventName.ON_CAN_PLAY_THROUGH);
                ...
                //ExternalInterface.call('videojs.Flash.test',   'xqg3');
          }
        }
        ...
        private function onNetStreamStatus(e:NetStatusEvent):void{
            switch(e.info.code){
                ...
                case "NetStream.Play.Start":
                    ...
                    //ExternalInterface.call('videojs.Flash.test',   'listenState-start');
                    if(_model.autoplay){
                        _model.broadcastEventExternally(ExternalEventName.ON_RESUME);
                ...
                case "NetStream.Buffer.Full":
                    ...
                    if (_isSeeking) {
                        ...
                        ExternalInterface.call('videojs.Flash.test',   'bufferFull');
                    }

                    _pausedSeekValue = -1;
                    _playbackStarted = true;
                    ExternalInterface.call('videojs.Flash.test',   'bufferFull2');
                    if(_pausePending){
                        _pausePending = false;
                        _ns.pause();
                        _isPaused = true;
                        ExternalInterface.call('videojs.Flash.test',   'bufferFull3');
                    } else if (_isBuffering) {
                        _model.broadcastEventExternally(ExternalEventName.ON_START);
                        ExternalInterface.call('videojs.Flash.test',   'bufferFull4');
                    }
                    _isBuffering = false;
                    ExternalInterface.call('videojs.Flash.test',   'bufferFull5');
                    break;
                ...
                case "NetStream.Buffer.Empty":
                    ...
                    if (_ending) {
                        ...
                        //ExternalInterface.call('videojs.Flash.test',   'bufferEmpty');
                        break;
                    }
                ...
                case "NetStream.Seek.Notify":
                    ...
                    _throughputTimer.reset();
                    _throughputTimer.start();
                    ExternalInterface.call('videojs.Flash.test',   'seek');
                    break;
        ...
        public function onMetaData(pMetaData:Object):void{
            ExternalInterface.call('videojs.Flash.test',   'xqg1seek', 'metadata');
            _metadata = pMetaData;
